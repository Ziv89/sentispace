...
const DatePicker = ({ label, date, onDateChange }: DatePickerProps) => {
  ...
  const handleModalOpen = (event: MouseEvent | TouchEvent | KeyboardEvent) => {
      ...
      {isModalOpen && (
        ...
        >
          <div className={classes.header}>
            <CaretLeft
              onClick={() => handleMonthNavigation(Directions.PREVIOUS)}
            />
            <div className={classes.displayedMonth}>
              {format(selectedMonth, 'MMMM yyyy')}
            </div>
            <CaretRight
              onClick={() => handleMonthNavigation(Directions.NEXT)}
            />
          </div>
          <div className={classes.month}>
            <div className={classes.week}>
              {weeksInMonths[0].map((day) => (
                <div
                  key={day.toString()}
            ...
            {weeksInMonths.map((week) => (
              <div key={week[0].toString()} className={classes.week}>
                {week.map((day) => (
                  <div
                    key={day.toString()}
                    className={cx({
